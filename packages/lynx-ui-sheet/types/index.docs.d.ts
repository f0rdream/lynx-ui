...
export interface SheetRootProps extends ComponentBasicProps {
    ...
    /**
     * Angle ranges (in degrees) where horizontal gestures should be ignored.
     * Each entry is [minAngle, maxAngle]. Gestures within these angle ranges
     * will not trigger sheet movement, allowing nested horizontal scrollers to work.
     * @example consumeSlideEvent={[[-135, -45], [45, 135]]} // Ignore left/right swipes
     * @zh 应忽略水平手势的角度范围（度数）。每个条目为 [最小角度, 最大角度]。
     * 在这些角度范围内的手势不会触发 Sheet 移动，允许嵌套的水平滚动组件正常工作。
     */
...
export interface SheetViewProps extends ComponentBasicProps {
     ...
     * @Harmony
     */
    sheetViewProps?: OverlayViewProps['overlayViewProps'];
}
