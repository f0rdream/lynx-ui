...
export function useSnap({
  ...
  const expandedIndex = snapOffsets.length > 0
    ? snapOffsets.indexOf(maxOffset)
    : 0
  const collapsedIndex = snapOffsets.length > 0
    ? snapOffsets.indexOf(minOffset)
    : 0
  ...
  function getClampedValueMT(targetY: number) {
    'main thread'
    return clamp(targetY, sheetHeightMTRef.current)
  }
  ...
  useMotionValueRefEvent(yRef, 'change', v => {
    ...
    if (el) {
      ...
      if (sheetProgress?.current) {
        ...
        const opened = offsets.length > 0 ? Math.max(...offsets) : 0
        const closed = 0
        let progress = 1
  ...
  function expandMT(opts?: SnappingOptions) {
    'main thread'
    controller.snapToMT(expandedIndex, opts)
  }

  function collapseMT(opts?: SnappingOptions) {
    'main thread'
    controller.snapToMT(collapsedIndex, opts)
  }
